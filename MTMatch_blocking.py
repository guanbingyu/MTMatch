...
# Native AMP is used instead of apex.amp.
...
def load_model(hp):
    ...
    # Keep the fp16/cuda guard for compatibility, but no apex initialization is needed.
    if hp.fp16 and 'cuda' in device:
        print("Using FP16/BFloat16 Mixed Precision (Native)")
        # Native autocast handles mixed precision; no model = amp.initialize(...) call is required.
...
if __name__ == '__main__':
    ...
    path = resolve_task_dir('em', hp.task)
